...
export default function CardItem({
  ...
  const countUp = () => {
    ...
    if (count !== limit) {
      ...
      setDisable(true);
      toast.error('최대 구매 가능 수량을 초과하였습니다.');
    }
  };
  ...
  const countDown = () => {
    if (count != 1) {
      ...
      toast.error('최소 1개 이상을 주문해야 합니다.');
    }
  };
        ...
        <div className="flex min-w-[288px] w-full h-full gap-4">
          ...
          <div className="flex items-end">
            <button
              ...
              className="text-xs border border-oguogu-gray-2 rounded-sm w-[28px] min-h-[40px] h-1/2 flex items-center justify-center gap-1 cursor-pointer"
            >
              <svg
